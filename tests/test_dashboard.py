...
class TestDashboard:
    ...
    def test_project_card_shows_description(self, page: Page, make_project):
        ...
        dashboard = DashboardPage(page)
        card = dashboard.get_project_card(name)
        expect(card.description.last).to_contain_text("Automated test")
    ...
    def test_edit_project_updates_name(self, page: Page, make_project):
        ...
        card.edit()
        expect(page.locator(DashboardPage.PROJECT_MODAL)).to_have_class(re.compile(r"is-open"))
        expect(page.locator(DashboardPage.PM_NAME)).to_have_value(name)

        page.locator(DashboardPage.PM_NAME).fill(name + " (edited)")
        page.locator(DashboardPage.PM_SUBMIT).click()
        expect(page.locator(DashboardPage.TOAST_SUCCESS).last).to_be_visible()
        expect(dashboard.get_project_card(name + " (edited)")._loc).to_be_visible()

    def test_delete_project_via_confirmation_modal(self, page: Page):
        dashboard = DashboardPage(page)
        dashboard.create_project("Delete Me Project")
        card = dashboard.get_project_card("Delete Me Project")
        card.delete(page.locator(DashboardPage.CONFIRM_OK))
        expect(page.locator(DashboardPage.TOAST_SUCCESS).last).to_be_visible()
        page.wait_for_load_state("networkidle")
        expect(dashboard.get_project_card("Delete Me Project")._loc).not_to_be_attached()
