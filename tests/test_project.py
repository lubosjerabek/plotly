...
class TestProjectDetail:
    ...
    def test_edit_phase_start_shifts_end_via_change_event(self, page: Page, make_project, make_phase):
        ...
        project.get_phase_card(phase_name).edit_btn.click()
        expect(page.locator(ProjectPage.GENERIC_MODAL)).to_have_class(re.compile(r"is-open"))
        # Simulate the native date picker: set value then dispatch 'change' (no 'input').
        # Shift start +10 days; end should follow: 2027-01-20 → 2027-01-30
        page.locator(ProjectPage.MODAL_START).evaluate(
            "el => { el.value = '2027-01-20'; el.dispatchEvent(new Event('change', { bubbles: true })); }"
        )
        expect(page.locator(ProjectPage.MODAL_END)).to_have_value("2027-01-30")
        page.keyboard.press("Escape")
    ...
    def test_edit_event_start_shifts_end_via_change_event(self, page: Page, make_project, make_phase):
        ...
        project = ProjectPage(page)
        # 6-day event: 2027-02-01 → 2027-02-07
        project.add_phase_event(phase_name, "Shift Me Event via Change", "2027-02-01", "2027-02-07")
        phase = project.get_phase_card(phase_name)
        phase.expand(page)
        phase.events_section().locator(".item-list li", has_text="Shift Me Event via Change") \
             .locator(ProjectPage.EDIT_EVENT_BTN).click()
        expect(page.locator(ProjectPage.GENERIC_MODAL)).to_have_class(re.compile(r"is-open"))
        # Simulate the native date picker: set value then dispatch 'change' (no 'input').
        # Shift start +7 days; end should follow: 2027-02-07 → 2027-02-14
        page.locator(ProjectPage.MODAL_START).evaluate(
            "el => { el.value = '2027-02-08'; el.dispatchEvent(new Event('change', { bubbles: true })); }"
        )
        expect(page.locator(ProjectPage.MODAL_END)).to_have_value("2027-02-14")
        page.keyboard.press("Escape")
