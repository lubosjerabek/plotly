...
class TestAdminAccess:
    ...
    def test_admin_panel_blocked_for_non_admin(self, second_user_page: Page):
        ...
        assert (
            "/admin/users" not in second_user_page.url
            or second_user_page.locator("text=403").count() > 0
            or second_user_page.locator("text=Forbidden").count() > 0
...
class TestAdminInvites:
    ...
    def test_revoke_invite_via_api(self, page: Page):
        ...
        revoke = page.request.delete(
            BASE_URL + f"/api/admin/invites/{invite_id}", headers={"X-Requested-With": "XMLHttpRequest"}
        )
